...
/* ---------------- UTF-8 -> Unicode (BMP) декодирование ----------------
 * Компонент из архива изначально работал только с ASCII (0..127),
 * так как обращался к font8x8_basic_tr[(uint8_t)text[i]].
 *
 * Для кириллицы в UTF-8 требуются:
 * 1) декодирование UTF-8 последовательностей в Unicode codepoint;
 * 2) таблица битмапов для нужного диапазона (см. font8x8_cyrillic.h);
 * 3) рендер символов по codepoint, а не по байту строки.
 */
static uint16_t ssd1306_utf8_decode_1(const char **pp, const char *end)
{
    ...
    // 2-byte
    if ((c0 & 0xE0) == 0xC0) {
        if ((p + 1) >= (const unsigned char *)end) { (*pp) += 1; return (uint16_t)'?'; }
        unsigned char c1 = p[1];
        if ((c1 & 0xC0) != 0x80) { (*pp) += 1; return (uint16_t)'?'; }
        uint16_t cp = (uint16_t)(((c0 & 0x1F) << 6) | (c1 & 0x3F));
        (*pp) += 2;
        return cp;
    }
    ...
    // 3-byte
    if ((c0 & 0xF0) == 0xE0) {
        if ((p + 2) >= (const unsigned char *)end) { (*pp) += 1; return (uint16_t)'?'; }
        unsigned char c1 = p[1];
        unsigned char c2 = p[2];
        if (((c1 & 0xC0) != 0x80) || ((c2 & 0xC0) != 0x80)) { (*pp) += 1; return (uint16_t)'?'; }
        uint16_t cp = (uint16_t)(((c0 & 0x0F) << 12) | ((c1 & 0x3F) << 6) | (c2 & 0x3F));
        (*pp) += 3;
        return cp;
    }

    // 4-byte и прочее здесь не поддерживаем (OLED 8x8 шрифт и так BMP).
    (*pp) += 1;
    return (uint16_t)'?';
}

static size_t ssd1306_utf8_to_codepoints(const char *text, int text_len, uint16_t *out, size_t out_cap)
{
    if (text == NULL || out == NULL || out_cap == 0) return 0;
    ...
    while (p < end && *p != '\0') {
        if (n >= out_cap) break;
        out[n++] = ssd1306_utf8_decode_1(&p, end);
    }
    return n;
}

static const uint8_t *ssd1306_glyph_from_codepoint(uint16_t cp)
{
    // ASCII (U+0000..U+007F)
    if (cp < 0x80) return font8x8_basic_tr[(uint8_t)cp];

    // Кириллица: Ё/ё
    if (cp == 0x0401) return font8x8_cyrillic_YO_upper;
    if (cp == 0x0451) return font8x8_cyrillic_yo_lower;

    // Кириллица: А..Я
    if (cp >= 0x0410 && cp <= 0x042F) return font8x8_cyrillic_upper[cp - 0x0410];

    // Кириллица: а..я
    if (cp >= 0x0430 && cp <= 0x044F) return font8x8_cyrillic_lower[cp - 0x0430];

    // Неизвестный символ
    return font8x8_basic_tr[(uint8_t)'?'];
}
...
void ssd1306_display_text(SSD1306_t * dev, int page, const char * text, int text_len, bool invert)
{
	...
	// Декодируем UTF-8 строку в массив Unicode codepoint'ов
	// text_len трактуем как длину в байтах (обычно передают strlen()).
	uint16_t cps[128];
	size_t cp_count = ssd1306_utf8_to_codepoints(text, text_len, cps, sizeof(cps)/sizeof(cps[0]));

	// Ограничение по ширине экрана: максимум 16 символов (16 * 8 = 128 px)
	if (cp_count > 16) cp_count = 16;

	int seg = 0;
	uint8_t image[8];
	for (size_t i = 0; i < cp_count; i++) {
		memcpy(image, ssd1306_glyph_from_codepoint(cps[i]), 8);
...
void ssd1306_display_text_box1(SSD1306_t * dev, int page, int seg, const char * text, int box_width, int text_len, bool invert, int delay)
{
	...
	// Заполняем бокс первыми box_width символами (или пробелами, если текста меньше)
	for (int i = 0; i < box_width; i++) {
		uint16_t cp = (i < (int)cp_count) ? cps[i] : (uint16_t)' ';
		memcpy(image, ssd1306_glyph_from_codepoint(cp), 8);
		if (invert) ssd1306_invert(image, 8);
		if (dev->_flip) ssd1306_flip(image, 8);
	...
	// Горизонтальный скролл внутри бокса
	for (int t = box_width; t < (int)cp_count; t++) {
		memcpy(image, ssd1306_glyph_from_codepoint(cps[t]), 8);
		if (invert) ssd1306_invert(image, 8);
		if (dev->_flip) ssd1306_flip(image, 8);
		for (int bit = 0; bit < 8; bit++) {
			for (int px = 0; px < text_box_pixel; px++) {
				dev->_page[page]._segs[px + seg] = dev->_page[page]._segs[px + seg + 1];
			}
			dev->_page[page]._segs[seg + text_box_pixel - 1] = image[bit];
...
void ssd1306_display_text_box2(SSD1306_t * dev, int page, int seg, const char * text, int box_width, int text_len, bool invert, int delay)
{
	...
	// Заполняем бокс пробелами
	for (int i = 0; i < box_width; i++) {
		memcpy(image, ssd1306_glyph_from_codepoint((uint16_t)' '), 8);
		if (invert) ssd1306_invert(image, 8);
		if (dev->_flip) ssd1306_flip(image, 8);
		ssd1306_display_image(dev, page, _seg, image, 8);
		_seg = _seg + 8;
	...
	// Горизонтальный скролл внутри бокса: выводим весь текст слева направо
	for (int t = 0; t < (int)cp_count; t++) {
		memcpy(image, ssd1306_glyph_from_codepoint(cps[t]), 8);
		if (invert) ssd1306_invert(image, 8);
		if (dev->_flip) ssd1306_flip(image, 8);
		for (int bit = 0; bit < 8; bit++) {
...
void ssd1306_display_text_x3(SSD1306_t * dev, int page, const char * text, int text_len, bool invert)
{
	...
	uint16_t cps[64];
	size_t cp_count = ssd1306_utf8_to_codepoints(text, text_len, cps, sizeof(cps)/sizeof(cps[0]));
	if (cp_count > 5) cp_count = 5;

	int seg = 0;

	for (size_t nn = 0; nn < cp_count; nn++) {

		uint8_t const * const in_columns = ssd1306_glyph_from_codepoint(cps[nn]);
		...
		// Draw twice in height (because pages)
		for (int k = 0; k < 2; k++) {
			for (int xx = 0; xx < 8; xx++) {
				uint8_t image[8];
				image[0] = (out_columns[xx].u8[0 + k * 2]);
				image[1] = (out_columns[xx].u8[1 + k * 2]);
				image[2] = 0x00;
				image[3] = 0x00;
				image[4] = 0x00;
				image[5] = 0x00;
				...
				if (invert) ssd1306_invert(image, 8);
				if (dev->_flip) ssd1306_flip(image, 8);
				ssd1306_display_image(dev, page + k, seg + xx, image, 2);
			}
		}
		seg = seg + 8;
	}
